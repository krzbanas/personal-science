...
math = false
+++
Numerical integration in R by fitting a piece-wise linear function. Example adapted from quoted sources. Firstly piece-wise linear function is fitted by 'approxfun', then integration is done by function 'integrate' over the original limits of x. Secondly smooth spline is tested (with the function 'splinefun'). Finally both solution are visually compared
